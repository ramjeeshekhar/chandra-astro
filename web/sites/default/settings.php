...
$databases['default']['default'] = [
  ...
  'host' => getenv('DB_HOST'), // Removed localhost fallback
  'port' => getenv('DB_PORT'),
  'prefix' => '',
  'isolation_level' => 'READ COMMITTED', //For better performance
];
...
$settings['hash_salt'] = getenv('DRUPAL_HASH_SALT') ?: 'local-only-fallback';
...
# $settings['trusted_host_patterns'] = [];
/**
 * Modified setting for trusted host patten in Server
 */

  $settings['trusted_host_patterns'] = [
    '^chandraastro\.in$',
    '^www\.chandraastro\.in$',
    '^[a-z0-9-]+\.chandraastro\.in$',
  ];
